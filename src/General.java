...
public class General {
	...
	private String q2 = "SELECT t1.ticker, t1.volumeTraded " +
			"FROM " +
			"(SELECT Ticker, SUM(p.volume) as volumeTraded " +
			"FROM AdjustedPrices p " +
			"WHERE YEAR(p.day) = 2016 " +
			"GROUP BY Ticker " +
			"ORDER BY volumeTraded DESC) t1, " +
			"(SELECT Ticker, SUM(p.volume) as volumeTraded " +
			"FROM AdjustedPrices p " +
			"WHERE YEAR(p.day) = 2016 " +
			"GROUP BY Ticker) t2 " +
			...
			"HAVING count(*) <= 10 " +
			"ORDER BY t1.volumeTraded DESC; ";

	private String q3 = "SELECT absYears.Year, absYears.Place, absYears.AbsoluteTicker,  " +
	...
	private String q5 = "SELECT Sector, Difference, Ratio, totalratio, totaldiff " +
			"FROM " +
